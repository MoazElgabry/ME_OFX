...
inline float3x3 identity() { return make_float3x3(make_float3(1.0f, 0.0f, 0.0f), make_float3(0.0f, 1.0f, 0.0f), make_float3(0.0f, 0.0f, 1.0f)); }
...
inline float3 operator+(float a, const float3& b) { return make_float3(a + b.x, a + b.y, a + b.z); }
inline float3 operator-(const float3& a, float b) { return make_float3(a.x - b, a.y - b, a.z - b); }
inline float3 operator-(float a, const float3& b) { return make_float3(a - b.x, a - b.y, a - b.z); }
inline float3 operator*(const float3& a, float b) { return make_float3(a.x * b, a.y * b, a.z * b); }
...
inline float spowf(float a, float b) { return (a <= 0.0f) ? a : std::pow(a, b); }
inline float3 spowf3(float3 a, float b) { return make_float3(spowf(a.x, b), spowf(a.y, b), spowf(a.z, b)); }
inline float hypotf2(float2 v) { return std::sqrt(std::fmax(0.0f, v.x * v.x + v.y * v.y)); }
inline float hypotf3(float3 v) { return std::sqrt(std::fmax(0.0f, v.x * v.x + v.y * v.y + v.z * v.z)); }
inline float3 clampf3(float3 a, float mn, float mx) { return make_float3(std::fmin(std::fmax(a.x, mn), mx), std::fmin(std::fmax(a.y, mn), mx), std::fmin(std::fmax(a.z, mn), mx)); }
inline float3 clampminf3(float3 a, float mn) { return make_float3(std::fmax(a.x, mn), std::fmax(a.y, mn), std::fmax(a.z, mn)); }
inline float exp10f_compat(float x) { return std::exp2(x * 3.3219280948873626f); }

inline float oetf_davinci_intermediate(float x) { return x <= 0.02740668f ? x / 10.44426855f : std::exp2(x / 0.07329248f - 7.0f) - 0.0075f; }
inline float oetf_filmlight_tlog(float x) { return x < 0.075f ? (x - 0.075f) / 16.184376489665897f : std::exp((x - 0.5520126568606655f) / 0.09232902596577353f) - 0.0057048244042473785f; }
inline float oetf_acescct(float x) { return x <= 0.155251141552511f ? (x - 0.0729055341958355f) / 10.5402377416545f : std::exp2(x * 17.52f - 9.72f); }
inline float oetf_arri_logc3(float x) { return x < 5.367655f * 0.010591f + 0.092809f ? (x - 0.092809f) / 5.367655f : (exp10f_compat((x - 0.385537f) / 0.247190f) - 0.052272f) / 5.555556f; }
inline float oetf_arri_logc4(float x) { return x < -0.7774983977293537f ? x * 0.3033266726886969f - 0.7774983977293537f : (std::exp2(14.0f * (x - 0.09286412512218964f) / 0.9071358748778103f + 6.0f) - 64.0f) / 2231.8263090676883f; }
inline float oetf_red_log3g10(float x) { return x < 0.0f ? (x / 15.1927f) - 0.01f : (exp10f_compat(x / 0.224282f) - 1.0f) / 155.975327f - 0.01f; }
inline float oetf_panasonic_vlog(float x) { return x < 0.181f ? (x - 0.125f) / 5.6f : exp10f_compat((x - 0.598206f) / 0.241514f) - 0.00873f; }
inline float oetf_sony_slog3(float x) { return x < 171.2102946929f / 1023.0f ? (x * 1023.0f - 95.0f) * 0.01125f / (171.2102946929f - 95.0f) : (exp10f_compat(((x * 1023.0f - 420.0f) / 261.5f)) * (0.18f + 0.01f) - 0.01f); }
inline float oetf_fujifilm_flog2(float x) { return x < 0.100686685370811f ? (x - 0.092864f) / 8.799461f : (exp10f_compat(((x - 0.384316f) / 0.245281f)) / 5.555556f - 0.064829f / 5.555556f); }
...
inline float3 eotf_hlg(float3 rgb, int inverse) {
  if (inverse == 1) {
    ...
    rgb.z = rgb.z <= 0.5f ? rgb.z * rgb.z / 3.0f : (std::exp((rgb.z - 0.55991073f) / 0.17883277f) + 0.28466892f) / 12.0f;
...
inline float compress_toe_cubic(float x, float m, float w, int inv) {
  ...
  float radicand = std::fmax(0.0f, x2 * p1 * p1 - 4.0f * p0 * p0 * p0);
  float base = std::sqrt(radicand) / 2.0f + x * p1 / 2.0f;
  float p2 = base <= 0.0f ? 0.0f : std::pow(base, 1.0f / 3.0f);
  if (p2 == 0.0f) return x;
  return p0 / (3.0f * p2) + p2 / 3.0f + x / 3.0f;
}
inline float softplus(float x, float s) { if (x > 10.0f * s || s < 1e-4f) return x; return s * std::log(std::fmax(0.0f, 1.0f + std::exp(x / s))); }
inline float gauss_window(float x, float w) { return std::exp(-x * x / w); }
inline float2 opponent(float3 rgb) { return make_float2(rgb.x - rgb.z, rgb.y - (rgb.x + rgb.z) / 2.0f); }
inline float hue_offset(float h, float o) { return std::fmod(h - o + PI, 2.0f * PI) - PI; }

inline float contrast_high(float x, float p, float pv, float pv_lx, int inv) {
  const float x0 = 0.18f * std::pow(2.0f, pv);
  if (x < x0 || p == 1.0f) return x;
  const float o = x0 - x0 / p;
  const float s0 = std::pow(x0, 1.0f - p) / p;
  const float x1 = x0 * std::pow(2.0f, pv_lx);
  const float k1 = p * s0 * std::pow(x1, p) / x1;
  const float y1 = s0 * std::pow(x1, p) + o;
  if (inv == 1) {
    if (x > y1) return (x - y1) / k1 + x1;
    float base = (x - o) / s0;
    return base <= 0.0f ? 0.0f : std::pow(base, 1.0f / p);
  }
  return x > x1 ? k1 * (x - x1) + y1 : s0 * std::pow(x, p) + o;
}
...
inline float3 transformPixel(int width, int height, int x, int y, float3 rgb, const OpenDRTParams* p, const OpenDRTDerivedParams* d) {
  ...
  float crv_tsn = 0.0f;
  float2 pos = make_float2(static_cast<float>(x), static_cast<float>(y));
  float2 res = make_float2(static_cast<float>(width), static_cast<float>(height));
  ...
  float3 rs_w = make_float3(p->rs_rw, 1.0f - p->rs_rw - p->rs_bw, p->rs_bw);
  float sat_L = rs_w.x * rgb.x + rs_w.y * rgb.y + rs_w.z * rgb.z;
  ...
  float hue = std::fmod(std::atan2(opp.x, opp.y) + PI + 1.10714931f, 2.0f * PI);
  ...
  if (p->brl_enable) {
    float brl_tsf = std::pow(tsn / (tsn + 1.0f), 1.0f - p->brl_rng);
    float brl_exf = (p->brl + p->brl_r * ha_rgb.x + p->brl_g * ha_rgb.y + p->brl_b * ha_rgb.z) * std::pow(ach_d, 1.0f / p->brl_st);
    float brl_ex = std::pow(2.0f, brl_exf * (brl_exf < 0.0f ? brl_tsf : 1.0f - brl_tsf));
    tsn *= brl_ex;
  }

  if (p->tn_lcon_enable) {
    float lcon_m = std::pow(2.0f, -p->tn_lcon), lcon_w = p->tn_lcon_w / 4.0f; lcon_w *= lcon_w;
  ...
  if (p->tn_hcon_enable) {
    const float hcon_p = std::pow(2.0f, p->tn_hcon);
  ...
  if (d->enabled != 0) {
    ts_m2 = d->ts_m2; ts_p = d->ts_p; ts_s1 = d->ts_s1; s_Lp100 = d->s_Lp100; ts_s = d->ts_s;
  } else {
    float ts_x1 = std::pow(2.0f, 6.0f * p->tn_sh + 4.0f), ts_y1 = p->tn_Lp / 100.0f, ts_x0 = 0.18f + p->tn_off;
    float ts_y0 = p->tn_Lg / 100.0f * (1.0f + p->tn_gb * std::log2(ts_y1));
    float ts_s0 = compress_toe_quadratic(ts_y0, p->tn_toe, 1);
    ts_p = p->tn_con / (1.0f + static_cast<float>(p->tn_su) * 0.05f);
    float ts_s10 = ts_x0 * (std::pow(ts_s0, -1.0f / p->tn_con) - 1.0f);
    float ts_m1 = ts_y1 / std::pow(ts_x1 / (ts_x1 + ts_s10), p->tn_con);
    ts_m2 = compress_toe_quadratic(ts_m1, p->tn_toe, 1);
    ts_s = ts_x0 * (std::pow(ts_s0 / ts_m2, -1.0f / p->tn_con) - 1.0f);
    s_Lp100 = ts_x0 * (std::pow((p->tn_Lg / 100.0f), -1.0f / p->tn_con) - 1.0f);
    float pt_cmp_Lf = p->pt_hdr * std::fmin(1.0f, (p->tn_Lp - 100.0f) / 900.0f);
  ...
  if (p->hc_enable) {
    float hc_ts = std::pow(1.0f - tsn_const, 1.0f / p->hc_r_rng);
  ...
  if (p->hs_rgb_enable) {
    float3 hs_rgb = make_float3(ha_rgb_hs.x * ach_d * std::pow(tsn_pt, 1.0f / p->hs_r_rng), ha_rgb_hs.y * ach_d * std::pow(tsn_pt, 1.0f / p->hs_g_rng), ha_rgb_hs.z * ach_d * std::pow(tsn_pt, 1.0f / p->hs_b_rng));
  ...
  if (p->hs_cmy_enable) {
    float t = 1.0f - tsn_pt;
    float3 hs_cmy = make_float3(ha_cmy.x * ach_d * std::pow(t, 1.0f / p->hs_c_rng), ha_cmy.y * ach_d * std::pow(t, 1.0f / p->hs_m_rng), ha_cmy.z * ach_d * std::pow(t, 1.0f / p->hs_y_rng));
  ...
  if (p->ptm_enable) {
    ...
    float high = (p->ptm_high_st == 0.0f || p->ptm_high_rng == 0.0f) ? 1.0f : 1.0f + p->ptm_high * std::exp(-2.0f * ach_d * ach_d / p->ptm_high_st) * std::pow(tsn_pt, 1.0f / (4.0f * p->ptm_high_rng));
    ptf *= low * high;
  }

  rgb = rgb * ptf + 1.0f - ptf;
  sat_L = rs_w.x * rgb.x + rs_w.y * rgb.y + rs_w.z * rgb.z;
  float inv_rs_denom = p->rs_sa - 1.0f;
  if (std::fabs(inv_rs_denom) < 1e-6f) inv_rs_denom = inv_rs_denom < 0.0f ? -1e-6f : 1e-6f;
  ...
  if (p->brlp_enable) {
    ...
    float ba = hypotf2(bo) / 4.0f;
    ba = 1.1f * (ba * ba / (ba + 0.1f));
    float3 bhrgb = ach_d * ha_rgb;
    float bm = p->brlp + p->brlp_r * bhrgb.x + p->brlp_g * bhrgb.y + p->brlp_b * bhrgb.z;
    rgb *= std::pow(2.0f, bm * ba * tsn);
  }
  if (p->ptl_enable) rgb = make_float3(softplus(rgb.x, p->ptl_c), softplus(rgb.y, p->ptl_m), softplus(rgb.z, p->ptl_y));
  ...
  if (p->crv_enable == 1) {
    ...
    crv_rgb_dst.x = std::exp(-crv_rgb_dst.x * crv_rgb_dst.x * crv_w0);
    crv_rgb_dst.y = std::exp(-crv_rgb_dst.y * crv_rgb_dst.y * crv_w0);
    crv_rgb_dst.z = std::exp(-crv_rgb_dst.z * crv_rgb_dst.z * crv_w0);
...
#undef matrix_arriwg3_to_xyz
#undef matrix_arriwg4_to_xyz
#undef matrix_redwg_to_xyz
#undef matrix_sonysgamut3_to_xyz
#undef matrix_sonysgamut3cine_to_xyz
#undef matrix_vgamut_to_xyz
#undef matrix_egamut_to_xyz
#undef matrix_egamut2_to_xyz
#undef matrix_davinciwg_to_xyz
#undef matrix_p3d65_to_xyz
#undef matrix_xyz_to_p3d65
#undef matrix_rec709_to_xyz
#undef matrix_xyz_to_rec709
#undef matrix_p3_to_rec2020
#undef matrix_cat_dci_to_d93
#undef matrix_cat_dci_to_d75
#undef matrix_cat_dci_to_d65
#undef matrix_cat_dci_to_d60
#undef matrix_cat_dci_to_d55
#undef matrix_cat_dci_to_d50
#undef matrix_cat_d65_to_d93
#undef matrix_cat_d65_to_d75
#undef matrix_cat_d65_to_d60
#undef matrix_cat_d65_to_d55
#undef matrix_cat_d65_to_d50
#undef matrix_cat_d65_to_dci
#undef matrix_cat_d60_to_d93
#undef matrix_cat_d60_to_d75
#undef matrix_cat_d60_to_d65
#undef matrix_cat_d60_to_d55
